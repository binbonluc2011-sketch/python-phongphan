...
def PT_bac_hai(parent):
    #tạo frame phương trình bậc hai
    pt_bac2 = tk.Frame(parent, width=parent.winfo_screenwidth()/2, bg="white")
    pt_bac2.pack(side="left", fill="y")
    pt_bac2.pack_propagate(False)
    #tạo title phương trình
    bac2_title = tk.Label(pt_bac2, text="PHƯƠNG TRÌNH BẬC HAI", font=("Arial", 16), bg="white")
    bac2_title.pack(side="top")
    #tạo label phương trình
    ptrinh_label = tk.Label(pt_bac2, text="phương trình bậc hai một ẩn: ax² + bx + c = 0, a khác 0", font=("Arial", 13), bg="white")
    ptrinh_label.place(relx=0.1, rely=0.1)
    #tạo input phương trình
    ptrinh_input = tk.Label(pt_bac2, text="nhập phương trình: ", font=("Arial", 11), bg="white")
    ...
    def button_clicked():
        ...
        try:
            if len(n0) == 0:
                vo_n0 = tk.Label(awnser_frame, text="Phương trình vô nghiệm", font=("Arial", 13), bg="white")
            ...
            syntax_error = tk.Label(awnser_frame, text="SYNTAX ERROR", color="red", bg="white", font=("Arial", 13))
